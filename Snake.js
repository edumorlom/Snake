export default class SnakeComponent {
    constructor(rows, columns) {
        ...
        this.alive = true;
        this.hold = false;
        this.eats = (direction, food) => {
            /**
             * Snake eats the food.
             * If the head is in the same position as the food, then it has eaten it.
             * Returns whether there was food at that location.
             */
            if (this.getHeadPosition().row != food.row)
                return false;
            if (this.getHeadPosition().column != food.column)
                return false;
            if (direction === 'up')
                this.bodyPosition = [...this.bodyPosition, { row: (this.bodyPosition[0].row - 1) % this.rows, column: (this.bodyPosition[0].column) % this.columns }];
            if (direction === 'down')
                this.bodyPosition = [...this.bodyPosition, { row: (this.bodyPosition[0].row + 1) % this.rows, column: (this.bodyPosition[0].column) % this.columns }];
            if (direction === 'left')
                this.bodyPosition = [...this.bodyPosition, { row: (this.bodyPosition[0].row) % this.rows, column: (this.bodyPosition[0].column - 1) % this.columns }];
            if (direction === 'right')
                this.bodyPosition = [...this.bodyPosition, { row: (this.bodyPosition[0].row) % this.rows, column: (this.bodyPosition[0].column + 1) % this.columns }];
            return true;
        };
        ...
        this.move = (direction) => {
            ...
            for (let i = 1; i < this.bodyPosition.length; i++) {
                if (this.getHeadPosition().column === this.bodyPosition[i].column &&
                    this.getHeadPosition().row === this.bodyPosition[i].row)
                    this.alive = false;
            }
        };
        this.moveUp = () => {
